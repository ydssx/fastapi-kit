...
class CreatorUsageService:
    ...
    def _limits(self, user: User) -> tuple[int, int, int]:
        ...
        playground_limit = self.settings.creator_free_playground_calls_per_month * multiplier
        return completed_limit, ai_limit, playground_limit
    ...
    async def get_usage(self, user: User) -> UsageOut:
        ...
        completed_limit, ai_limit, playground_limit = self._limits(user)
        return UsageOut(
            ...
            playground_calls=counter.playground_calls,
            playground_calls_limit=playground_limit,
            plan=user.plan,
        )
    ...
    async def check_playground_quota(self, user: User) -> None:
        usage = await self.get_usage(user)
        if usage.playground_calls >= usage.playground_calls_limit:
            raise AppException(
                "Monthly Playground limit reached. Upgrade to Pro for more.",
                code=40203,
                status_code=402,
            )
    ...
    async def increment_playground(self, user: User) -> None:
        counter = await self.usage.get_or_create(user.id, self.current_year_month())
        counter.playground_calls += 1
        await self.usage.save(counter)
