...
describe('creator api client contract', () => {
  beforeEach(() => {
    localStorage.clear()
    fetchMock.mockReset()
  ...
  it('sends JSON requests and unwraps successful envelopes', async () => {
    ...
    await expect(
      apiFetch<{ id: number }>('/api/v1/creator/example', {
        method: 'POST',
  ...
  it('does not set Content-Type for FormData', async () => {
    ...
    await apiFetch('/api/v1/creator/upload', { method: 'POST', body: form })
  ...
  it('refreshes once before parsing a 401 response and retries the request', async () => {
    ...
    await expect(apiFetch<{ id: number }>('/api/v1/creator/example')).resolves.toEqual({
      id: 8,
    })
  ...
  it('clears tokens when refresh fails without recursively refreshing', async () => {
    ...
    await expect(apiFetch('/api/v1/creator/example')).rejects.toMatchObject({
      name: 'ApiError',
      code: 40100,
      status: 401,
    })
  ...
  it('returns undefined for 204 responses', async () => {
    ...
    await expect(
      apiFetch<void>('/api/v1/creator/example', { method: 'DELETE' }),
  ...
  it('throws ApiError for error envelopes', async () => {
    ...
    const request = apiFetch('/api/v1/creator/example')
  ...
  it('allows null data under the compatible creator policy', async () => {
    fetchMock.mockResolvedValueOnce(
      jsonResponse({ code: 0, message: 'ok', data: null }),
    )

    await expect(apiFetch<null>('/api/v1/creator/example')).resolves.toBeNull()
  })
})
